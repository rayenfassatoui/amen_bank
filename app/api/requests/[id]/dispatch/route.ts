...
import { confirmDispatchSchema } from "@/lib/validations/request"
...
export async function PATCH(
  ...
  try {
    ...
    const validation = confirmDispatchSchema.safeParse(body)
    ...
    if (fundRequest.status !== "ASSIGNED") {
      return NextResponse.json(
        {
          status: "error",
          message: `Cannot dispatch request with status: ${fundRequest.status}`,
        },
        { status: 400 }
      )
    }
    ...
    const updatedRequest = await prisma.$transaction(async (tx) => {
      const updated = await tx.request.update({
        ...
        data: {
          status: "DISPATCHED",
          dispatchedBy: validation.data.dispatchedBy,
      ...
      await tx.actionLog.create({
        data: {
          ...
          requestId: id,
          details: `Dispatched by ${validation.data.dispatchedBy}`,
        },
      })
